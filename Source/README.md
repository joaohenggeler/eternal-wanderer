...
* It's also recommended that you adjust the scale filter dimensions in the `ffmpeg_upload_output_args` option depending on your screen capture dimensions (or display settings if these are set to null in `screen_capture_recorder_settings`). For example, you could change the scale filter width and height to 1920x1080 (16:9) or 1440x1080 (4:3) in order to record 1080p videos.
